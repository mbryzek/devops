...
3. `umask 077 && $EDITOR ~/.platform/product-owner-credentials.json` — the logins
   the daily `product-owner` producer reviews the products with. One object per
   app, each with `url`, `username`, `password`; `acumen` is the only one needed
   (Playbook mints a login token from the AI token instead of storing a
   password). `dev agent doctor` reports it under "Product-owner review
   credentials", and `Agent::ProductOwner::REQUIRED_APPS` is the list.
4. **`dev agent doctor` — it must exit 0**
5. `cp launchd/com.bryzek.dev-agent.plist ~/Library/LaunchAgents/`
6. `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.bryzek.dev-agent.plist`
...
Step 3 is the one manual secret on the box, and it is deliberately *not* part of
the doctor's exit code: a runner without it can still claim every issue that is
not a product review, which is the same reasoning `Agent::Credentials` uses at
`lib/agent/credentials.rb:96-102`. It is also the one thing under `~/.platform`
that cannot be regenerated by re-registering, which is why ISS-1109 proposes
moving it and every other agent credential into `~/code/env/agents` so runners
can sync rather than each be set up by hand.

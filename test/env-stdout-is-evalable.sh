...
# Regression guard for `bin/env --format sh`: its stdout is eval'd by the
...
out=$("$DIR/bin/env" --app platform --env development --format sh 2>/dev/null)

# Every line of stdout must be shell we can eval; a stray banner is the failure
# this guards against.
if printf '%s\n' "$out" | grep -q '^==>'; then
  echo "FAIL: bin/env wrote a '==>' progress banner to stdout"
  exit 1
fi

eval "$out true"

echo "PASS: bin/env stdout is clean and evalable"

...
# The lake's file order is not its version order: a lexical sort puts 0.0.10 before
# 0.0.2, and releases must go up in version order or the server's max_version stops
# being a valid resumable cursor.
class TestChangelogLakeVersions < Minitest::Test
  def setup
    @dir = Dir.mktmpdir("changelog-lake")
    %w[0.0.1 0.0.2 0.0.10 0.0.11].each { |v| File.write(File.join(@dir, "#{v}.tag.json"), "{}") }
  end

  def teardown = FileUtils.remove_entry(@dir)

  def test_orders_by_the_tag_list_not_lexically
    order = %w[0.0.1 0.0.2 0.0.10 0.0.11]
    assert_equal order, changelog_lake_versions(@dir, ".tag.json", order)
  end

  # No checkout means no ordering to borrow. Falling back to the lexical order is wrong
  # but harmless (the upsert dedupes); silently dropping the files would not be.
  def test_no_tag_list_keeps_every_version
    assert_equal %w[0.0.1 0.0.10 0.0.11 0.0.2], changelog_lake_versions(@dir, ".tag.json", [])
  end

  def test_a_version_with_no_tag_sorts_last
    File.write(File.join(@dir, "9.9.9.tag.json"), "{}")
    assert_equal "9.9.9", changelog_lake_versions(@dir, ".tag.json", %w[0.0.1 0.0.2 0.0.10 0.0.11]).last
  end

  def test_only_the_named_suffix
    File.write(File.join(@dir, "0.0.1.notes.json"), "{}")
    assert_equal %w[0.0.1], changelog_lake_versions(@dir, ".notes.json", %w[0.0.1])
  end
end

class TestChangelogLakePendingReleases < Minitest::Test
  ORDER = %w[0.0.1 0.0.2 0.0.10 0.0.11].freeze

  def test_sends_only_what_is_past_the_cursor
    assert_equal %w[0.0.10 0.0.11], changelog_lake_pending_releases(ORDER, ORDER, "0.0.2")
  end

  # The steady state: the lake is entirely below a server that capture has moved on
  # past. Re-posting it every run was the bug.
  def test_a_lake_below_the_cursor_sends_nothing
    assert_empty changelog_lake_pending_releases(%w[0.0.1 0.0.2], ORDER, "0.0.11")
  end

  def test_an_empty_server_gets_everything
    assert_equal ORDER, changelog_lake_pending_releases(ORDER, ORDER, nil)
  end

  # A deleted tag must not silently truncate the send: resending is a no-op, skipping is
  # not.
  def test_an_unrecognized_cursor_sends_everything
    assert_equal ORDER, changelog_lake_pending_releases(ORDER, ORDER, "0.0.99")
  end
end

# push exists to move the lake into platform, and its counts are the only signal that
# the move is done. Exercises the real cmd_changelog_push with the API, the lake clone
# and the checkout stubbed.
class TestDevChangelogPush < Minitest::Test
  def setup
    @lake = Dir.mktmpdir("changelog-lake")
    @app_dir = File.join(@lake, "playbook-admin")
    FileUtils.mkdir_p(@app_dir)
    # Pipeline mode checks for a real clone before doing anything.
    FileUtils.mkdir_p(File.join(@lake, ".git"))
    @posted = []
    @max_version = nil
    @missing = []
    @status_reads = 0
  ...
  def lake_release(version, previous = nil)
    File.write(File.join(@app_dir, "#{version}.tag.json"), JSON.generate(
      "version" => version, "previous_version" => previous, "repo" => "playbook-admin",
      "released_at" => "2026-07-20T14:00:00-04:00",
      "commits" => [{ "sha" => "a1", "subject" => "Did a thing", "body" => "", "pr_number" => 7 }]
    ))
  end

  def lake_notes(version, entries = [{ "summary" => "Did a thing", "category" => "fix" }])
    File.write(File.join(@app_dir, "#{version}.notes.json"),
               JSON.generate("version" => version, "entries" => entries))
  end
  ...
  def stub_world!
    ...
    stub_global(:platform_endpoint) { |_localhost| { name: "test" } }
    stub_global(:ensure_changelog_repo!) { lake }
    stub_global(:changelog_repo_dir) { lake }
    stub_global(:changelog_app_repo) { |_app| "playbook-admin" }
    stub_global(:changelog_checkout_tags) { |_app| test.order }
    stub_global(:changelog_status) do |_endpoint|
      test.record_status_read
      { "playbook-admin" => { "application" => "playbook-admin",
                              "max_version" => test.max_version,
                              "versions_missing_notes" => test.missing.dup } }
    end
    stub_global(:changelog_post_batches) do |_endpoint, path, _key, records|
      posted << [path, records]
      records.length
  ...
  attr_reader :max_version, :missing

  def order = %w[0.3.0 0.3.1 0.3.2]

  def record_status_read = @status_reads += 1

  def push(*args)
    out, = capture_io { cmd_changelog_push(["--app", "playbook-admin"] + args) }
    out
  end

  def posted_for(kind) = @posted.select { |path, _| path == kind }.flat_map { |_, records| records }

  # The bug this fixes: a caught-up server used to be handed all 1190 lake records
  # every run, and reported the file count as if it were a delta.
  def test_a_caught_up_server_gets_nothing
    %w[0.3.0 0.3.1].each { |v| lake_release(v); lake_notes(v) }
    @max_version = "0.3.2"
    out = push
    assert_empty posted_for("releases")
    assert_empty posted_for("notes")
    assert_includes out, "changelog push: sent 0 release(s) and 0 notes from 1 app(s)"
    assert_includes out, "  playbook-admin: already up to date"
  end

  def test_sends_only_releases_past_the_cursor
    %w[0.3.0 0.3.1 0.3.2].each { |v| lake_release(v) }
    @max_version = "0.3.0"
    push
    assert_equal %w[0.3.1 0.3.2], posted_for("releases").map { |r| r["version"] }
  end

  # Notes are an exact delta, not a watermark: the server names the versions it needs.
  def test_sends_notes_only_for_versions_the_server_says_are_missing
    %w[0.3.0 0.3.1 0.3.2].each { |v| lake_release(v); lake_notes(v) }
    @max_version = "0.3.2"
    @missing = %w[0.3.1]
    push
    assert_equal %w[0.3.1], posted_for("notes").map { |n| n["version"] }
  end

  # Notes for a release recorded moments ago: the pre-send status cannot list it, so the
  # cursor has to be re-read or those notes are skipped on every single run.
  def test_rereads_the_cursor_after_sending_releases
    lake_release("0.3.1")
    lake_notes("0.3.1")
    @max_version = "0.3.0"
    stub_global(:changelog_post_batches) do |_endpoint, path, _key, records|
      @posted << [path, records]
      @missing = %w[0.3.1] if path == "releases"
      records.length
    end
    push
    assert_equal 2, @status_reads, "the cursor must be re-read after releases are sent"
    assert_equal %w[0.3.1], posted_for("notes").map { |n| n["version"] }
  end

  def test_does_not_reread_the_cursor_when_nothing_was_sent
    @max_version = "0.3.2"
    push
    assert_equal 1, @status_reads
  end

  # The lake stores "" for a bodyless commit; the API models it as absent.
  def test_an_empty_body_is_omitted
    lake_release("0.3.1")
    @max_version = "0.3.0"
    push
    refute posted_for("releases")[0]["commits"][0].key?("body")
  end

  def test_a_corrupt_file_is_skipped_not_fatal
    lake_release("0.3.1")
    File.write(File.join(@app_dir, "0.3.2.tag.json"), "{not json")
    @max_version = "0.3.0"
    push
    assert_equal %w[0.3.1], posted_for("releases").map { |r| r["version"] }
  end

  def test_an_app_with_no_lake_directory_is_reported
    out, = capture_io { cmd_changelog_push(["--app", "playbook-www"]) }
    assert_includes out, "  playbook-www: no directory in the lake, skipping"
    assert_empty posted_for("releases") + posted_for("notes")
  end

  # Inside `dev changelog` push must never clone: most machines have no lake, and the
  # post-release hook is not a place to clone an archived repo from.
  def test_the_pipeline_never_clones_a_missing_lake
    cloned = false
    stub_global(:ensure_changelog_repo!) { cloned = true; lake }
    stub_global(:changelog_repo_dir) { File.join(@lake, "does-not-exist") }
    out, = capture_io { cmd_changelog_push(["--app", "playbook-admin"], pipeline: true) }
    refute cloned, "the pipeline must not clone the lake"
    assert_includes out, "no lake at"
    assert_empty posted_for("releases") + posted_for("notes")
  end

  # Standalone, a failed push is a failed command.
  def test_an_api_failure_exits_on_its_own
    stub_global(:changelog_status) { |_e| raise ApiError, "HTTP 500" }
    assert_raises(SystemExit) { capture_io { cmd_changelog_push(["--app", "playbook-admin"]) } }
  end

  # In the pipeline it is not: push backfills an archived repo, and `build` still has
  # notes to write.
  def test_an_api_failure_is_survivable_in_the_pipeline
    stub_global(:changelog_status) { |_e| raise ApiError, "HTTP 500" }

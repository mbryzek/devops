...
- **Releases**: `dev pending` reports what needs a release; `dev versions` reads the live
  version of each deployable app (`/_internal_/version` or the live k8s image tag) — the same
  signal `dev issue reconcile` uses.
...
- **Read the live state before changing anything.** Confirm with `kubectl` / `doctl` / `dev
  versions` what is actually running; the repo and the cluster drift.

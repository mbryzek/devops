...
module Agent
  module Paths
    ...
    # Logins the `product-owner` producer needs to review the products as a real
    # user (Agent::ProductOwner). Unlike everything else under state_dir this is
    # NOT derived and cannot be regenerated by re-registering — it has to be
    # provisioned. ISS-1109 tracks moving it, and every other agent credential,
    # into ~/code/env/agents so runners can sync rather than each be set up by
    # hand.
    def product_owner_credentials_file = File.join(state_dir, "product-owner-credentials.json")

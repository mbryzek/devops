      ...
      Array(found).each do |f|
        if f.present?
          ...
          lines << "  **Never print, echo, commit, or paste it** into a PR, an issue comment, a plan or a " \
                   "test fixture — and never write the value into a command line. `ps` shows every " \
                   "argument of every process to every sibling session on this runner; `exec` keeps the " \
                   "value in an environment instead, for one command's lifetime. See §4."
        else

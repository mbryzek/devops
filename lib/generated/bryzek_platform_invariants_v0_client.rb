...
# Service version: 0.7.35
# User agent: apibuilder app.apibuilder.io/bryzek/platform-invariants/latest/ruby_client
        ...
        module V0
          ...
          class Client
            ...
            module Constants
              ...
              NAMESPACE = 'com.bryzek.platform.invariants.v0' unless defined?(Constants::NAMESPACE)
              USER_AGENT = 'apibuilder app.apibuilder.io/bryzek/platform-invariants/latest/ruby_client' unless defined?(Constants::USER_AGENT)
              VERSION = '0.7.35' unless defined?(Constants::VERSION)
            ...
            def invariant_results
              @invariant_results ||= ::Com::Bryzek::Platform::Invariants::V0::Clients::InvariantResults.new(self)
            end
          end
          ...
          module Clients
            ...
            class InvariantResults
              ...
              def initialize(client)
                @client = HttpClient::Preconditions.assert_class('client', client, ::Com::Bryzek::Platform::Invariants::V0::Client)
              end

              def get_json
                r = @client.request("/_internal_/invariants.json").get
                ::Com::Bryzek::Platform::Invariants::V0::Models::InvariantResult.new(r)
              end
          ...
          module Models
            ...
            # An invariant that found issues (count > 0)
            class InvariantError

              attr_reader :name, :count, :execution_time_ms, :query, :examples

              def initialize(incoming={})
                opts = HttpClient::Helper.symbolize_keys(incoming)
                HttpClient::Preconditions.require_keys(opts, [:name, :count, :execution_time_ms, :query], 'InvariantError')
                @name = HttpClient::Preconditions.assert_class('name', opts.delete(:name), String)
                @count = HttpClient::Preconditions.assert_class('count', opts.delete(:count), Integer)
                @execution_time_ms = HttpClient::Preconditions.assert_class('execution_time_ms', opts.delete(:execution_time_ms), Integer)
                @query = HttpClient::Preconditions.assert_class('query', opts.delete(:query), String)
                @examples = (x = opts.delete(:examples); x.nil? ? nil : HttpClient::Preconditions.assert_class('examples', x, Array).map { |v| HttpClient::Preconditions.assert_class('examples', v, String) })
              end
              ...
              def copy(incoming={})
                InvariantError.new(to_hash.merge(HttpClient::Helper.symbolize_keys(incoming)))
              end
              ...
              def to_hash
                {
                  ...
                  :query => query,
                  :examples => examples.nil? ? nil : examples
                }
              end

            end

            # An invariant that failed due to an error during execution
            class InvariantFailure

              attr_reader :name, :error, :execution_time_ms

              def initialize(incoming={})
                opts = HttpClient::Helper.symbolize_keys(incoming)
                HttpClient::Preconditions.require_keys(opts, [:name, :error, :execution_time_ms], 'InvariantFailure')
                @name = HttpClient::Preconditions.assert_class('name', opts.delete(:name), String)
              ...
              def copy(incoming={})
                InvariantFailure.new(to_hash.merge(HttpClient::Helper.symbolize_keys(incoming)))
              end
            ...
            class InvariantResult
              ...
              attr_reader :non_zero, :error, :success

              def initialize(incoming={})
                opts = HttpClient::Helper.symbolize_keys(incoming)
                HttpClient::Preconditions.require_keys(opts, [:non_zero, :error, :success], 'InvariantResult')
                @non_zero = HttpClient::Preconditions.assert_class('non_zero', opts.delete(:non_zero), Array).map { |v| (x = v; x.is_a?(::Com::Bryzek::Platform::Invariants::V0::Models::InvariantError) ? x : ::Com::Bryzek::Platform::Invariants::V0::Models::InvariantError.new(x)) }
                @error = HttpClient::Preconditions.assert_class('error', opts.delete(:error), Array).map { |v| (x = v; x.is_a?(::Com::Bryzek::Platform::Invariants::V0::Models::InvariantFailure) ? x : ::Com::Bryzek::Platform::Invariants::V0::Models::InvariantFailure.new(x)) }
                @success = HttpClient::Preconditions.assert_class('success', opts.delete(:success), Array).map { |v| (x = v; x.is_a?(::Com::Bryzek::Platform::Invariants::V0::Models::InvariantSuccess) ? x : ::Com::Bryzek::Platform::Invariants::V0::Models::InvariantSuccess.new(x)) }
              end
              ...
              def copy(incoming={})
                InvariantResult.new(to_hash.merge(HttpClient::Helper.symbolize_keys(incoming)))
              end

              def to_hash
                {
                  :non_zero => non_zero.map { |o| o.to_hash },
                  :error => error.map { |o| o.to_hash },
                  :success => success.map { |o| o.to_hash }
                }
              end

            end

            # An invariant that passed (count = 0)
            class InvariantSuccess

              attr_reader :name, :execution_time_ms

              def initialize(incoming={})
                opts = HttpClient::Helper.symbolize_keys(incoming)
                HttpClient::Preconditions.require_keys(opts, [:name, :execution_time_ms], 'InvariantSuccess')
                @name = HttpClient::Preconditions.assert_class('name', opts.delete(:name), String)
                @execution_time_ms = HttpClient::Preconditions.assert_class('execution_time_ms', opts.delete(:execution_time_ms), Integer)
              end
              ...
              def copy(incoming={})
                InvariantSuccess.new(to_hash.merge(HttpClient::Helper.symbolize_keys(incoming)))
              end
